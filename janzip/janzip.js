...
var Zip = function () {
    ...
    var exp = (function () {
        ...
        // @todo replace this by eventemitter
        var callbacks = {};
        var maxevents = 10;
        var evix = 0;
        ...
        function getFileHeader (file, compressIndicator, compressedData) {
            ...
            // crc32
            header.writeInt32(0); //crc32(compressedData));
            // compressed size
            header.writeInt32(0); //compressedData.length);
            // uncompressed size
        ...
        function toBuffer (zipMethod) {
            ...
            // dirs have to get processed after the files
            var dirs = [];
            ...
            function writeFiles () {
                ...
                async.forEachLimit(files, 1, function (file, next) {
                    ...
                    if (err) {
                        return dispatchEvent("error", err);
                    }
            ...
            function writeDirs () {
                console.log("calling writedirs", dirs.length);
                ...
                //async.forEachLimit(dirs, 1, function (dir, next) {
                    ...
                    // file name
                    dirBuffer.write(dir.fileName, "ascii");
                    
                    console.log("dispatch from writedir");
            ...
            function writeEof () {
                ...
                dirEnd.writeInt16(files.length);
                dirEnd.writeInt16(files.length);
                // directory lengths
                ...
                console.log("dispatch from writeeof");
                dispatchEvent("data", dirEnd.getInternalBuffer());
                dispatchEvent("close");
            }
...
var store = (function () {
   return {
       indicator : [ 0x00, 0x00 ],
       compress : function (filename, data, error, close) {
...
function Crc32a () {
    ...
    this.onData = function (buf) {
        for (var ix = 0; ix < buf.length; ix++) {
            var offset = (this.crc ^ buf[ix]) & 0xFF;
            this.crc = (this.crc >>> 8) ^ table[offset];
        }    
    };

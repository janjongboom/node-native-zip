...
var Zip = function () {
    ...
    var exp = (function () {
         ...
         * Generate a file header (as a buffer)
         */
        function getFileHeader2(file, compressIndicator, compressedData) {
            var dt = getDateTimeHeaders(new Date());
        ...
        function toBuffer (zipMethod) {
            ...
            files.map(function(file) {
                ...
                // now create dir
                var fileHeader2 = getFileHeader2(file, zipMethod.indicator, data);
                var dirBuffer = new RollingBuffer(4 + 2 + fileHeader2.length + 6 + 4 + 4 + file.name.length);
                writeBytes(dirBuffer, [0x50, 0x4b, 0x01, 0x02]);
                writeBytes(dirBuffer, [0x14, 0x00]);
                dirBuffer.appendBuffer(fileHeader2);

...
{
    if (!JSZip.compressions.length) registerCompressionMethods();
...
function registerCompressionMethods () {
    
    var compression = [ require("./jszip.deflate") ];
    
    for (var ix = 0; ix < compression.length; ix++) {
        var c = compression[ix];
        
        JSZip.compressions[c.name] = c;
    }
